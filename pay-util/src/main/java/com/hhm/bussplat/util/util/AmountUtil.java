...
public class AmountUtil {
     ...
     * @return 两个参数的差
     */
    public static double subtract(double oneAmount, double twoAmount, int scale, RoundingMode roundingMode) {
        if (scale < 0) throw new IllegalArgumentException("The scale must be a positive integer or zero");
        return BigDecimal.valueOf(oneAmount).subtract(BigDecimal.valueOf(twoAmount)).setScale(scale, roundingMode).doubleValue();
    }
     ...
     * @return 两个参数的积
     */
    public static double multiply(double oneAmount, double twoAmount, int scale, RoundingMode roundingMode) {
        if (scale < 0) throw new IllegalArgumentException("The scale must be a positive integer or zero");
        return BigDecimal.valueOf(oneAmount).multiply(BigDecimal.valueOf(twoAmount)).setScale(scale, roundingMode).doubleValue();
    }
     ...
     * @param twoAmount 乘数
     * @param scale     保留几位小数
     * @param round     小数位处理方式 参考 BigDecimal.ROUND_UP
     * @return 两个参数的积
     */
    public static double multiply(double oneAmount, double twoAmount, int scale, int round) {
        if (scale < 0) throw new IllegalArgumentException("The scale must be a positive integer or zero");
        return BigDecimal.valueOf(oneAmount).multiply(BigDecimal.valueOf(twoAmount)).setScale(scale, round).doubleValue();
    }
    ...
    public static void main(String args[]) {
        System.out.println(formatToString(new BigDecimal(0.09)));
        System.out.println(formatToString(new BigDecimal(0.19)));
        System.out.println(formatToString(new BigDecimal(1.09)));
        System.out.println(formatToString(new BigDecimal(10.09)));
    }
}
