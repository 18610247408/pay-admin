...
<div class="row" style="height:330px;overflow: auto;">
    ...
    <div style="width:20px;  position: absolute; top: 138px; left:240px;">
        <a id="selectRoleRight" title="右移" style="cursor: pointer"><i class="layui-icon" style="color:#000000;">&#xe602;</i></a>
        <a id="selectRoleLeft"  title="左移" style="cursor: pointer"><i class="layui-icon" style="color:#000000;">&#xe603;</i></a>
    </div>
...
<input id="userIds"  name="id" type="hidden" value="${operatorId}">
...
<script type="text/javascript">
    layui.config({
        ...
        function getUndistributedRoleList(){
            ...
            $.ajax({
                url : '${ctx}/operator/ajaxUndistributedRoleList.do',
                type : 'post',
                async: false,
                data :{
                    operatorId: $("#userIds").val()
                },
                    ...
                    if (data != "") {
                        ...
                        $(pdata).each(function(index,item){
                            $("#undistributedRole").append(
                                    '<tr>'+
                                    '<td><input name="undistributed" lay-skin="primary" type="checkbox" alt="'+item.name+'"  value="'+item.id+'"></td>'+
                                    '<td>'+item.name+'</td>'+
                                    '</tr>'
                            );
                            form.render();
        ...
        function getDeceasedRolelist(){
            var pageLoading = layer.load(2);
            $.ajax({
                url : '${ctx}/operator/ajaxDeceasedRoleList.do',
                type : 'post',
                async: false,
                data :{
                    operatorId: $("#userIds").val()
                ...
                success : function(data) {
                    if(data != "" ){
                        ...
                        $(pdata).each(function(index,item){
                            $("#deceasedRole").append(
                                    '<tr>'+
                                    '<td><input name="deceased" lay-skin="primary" type="checkbox" alt="'+item.name+'" value="'+item.id+'"></td>'+
                                    '<td>'+item.name+'</td>'+
                                    '</tr>'
                            );
                            form.render();
        ...
        $("#saveRole").click(function(){
            ...
            $.ajax({
                url : '${ctx}/operator/ajaxSaveOperatorRole.do',
                type : 'post',
                async: false,
                data : {
                    operatorId:$('#userIds').val(),
